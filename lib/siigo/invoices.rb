class Siigo::Invoices

    def fetch(token, output_path)
        resource_tag = "invoices"
        ...
        headers = ["id", "number", "date", "customer",
                   "seller", "cost_center", "total", "created_at"]
        CSV.open(output_path, "w") do |csv|
            csv << headers
        
            resources_all_local.each do |resource_local|
                csv << [String(resource_local["id"]), 
                        String(resource_local["number"]), 
                        String(resource_local["date"]),
                        String(resource_local["customer"]["id"]),
                        String(resource_local["seller"]),
                        String(resource_local["cost_center"]),
                        String(resource_local["total"]),
