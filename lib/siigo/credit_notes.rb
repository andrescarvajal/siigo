class Siigo::CreditNotes

    def fetch(token, output_path)
        resource_tag = "credit-notes"
        ...
        headers = ["id", "number", "date", "type", 
                   "customer", "items", "payment", "created_at"]
        CSV.open(output_path, "w") do |csv|
            ...
            resources_all_local.each do |resource_local|
                csv << [String(resource_local["id"]), 
                        ...
                        String(resource_local["items"]),
                        String(resource_local["payment"]),
